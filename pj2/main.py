...
def model_testing(
    model, device, test_dataloader, test_acc, test_losses, misclassified=[]
):
    ...
    with torch.no_grad():
        ...
        for index, (data, target) in enumerate(test_dataloader):
            ...
            test_loss += torch.nn.CrossEntropyLoss()(output, target).item() * data.size(
                0
            )
    ...
    test_acc.append(100.0 * correct / len(test_dataloader.dataset))
...
def main():
    ...
    train_transforms = transforms.Compose(
        [
            ...
            transforms.Normalize(
                (0.491, 0.482, 0.446), (0.247, 0.243, 0.261)
    ...
    # TODO,分析讨论其他的CNN网络设计
    model = MyResNet50.resnet50().to(device)
    model = MyResNext.resnext50_32x4d().to(device)
    model = MyDenseNet.design_model().to(device)
